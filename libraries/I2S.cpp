...
I2S_Tx::I2S_Tx(uint32_t* reservedMem, uint8_t depth) : 
        WS_frame_size(0) {
    ...
    setReservedMem(reservedMem, depth);
}

void I2S_Tx::setReservedMem(uint32_t* reservedMem, uint8_t depth) {
    txPingPong.setReservedSpace(reservedMem, depth);
}
